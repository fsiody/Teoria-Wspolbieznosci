...
public class Checker {
    ...
    public static void main(String args[]) throws FileNotFoundException, IOException {
        ...
        double[][]lh=new double[source.size][source.size];
        double[][]rh=new double[source.size][1];
        for(int i=0;i<source.size;i++){
            for(int j=0;j<source.size;j++){
                lh[i][j]=source.lhs[i][j].doubleValue();
            }
            rh[i][0]=source.rhs[i][0].doubleValue();
        }
        Matrix A = new Matrix(lh);
        Matrix b = new Matrix(rh);
        // x=a/b

        source.printMatrix();
        Matrix x = A.solve(b);
        source.solve();     // my code
        ...
        System.out.print(String.format("%.1f",x.getArray()[0][0]));
        for (int j = 1; j < processed.size; j++) {
            System.out.print(" ");
            System.out.print(String.format("%.1f",x.getArray()[j][0]));
        }
        System.out.println();

        System.out.print(String.format("%.1f",processed.rhs[0][0]));
        for (int j = 1; j < processed.size; j++) {
            System.out.print(" ");
            System.out.print(String.format("%.1f",processed.rhs[j][0]));
        }
        System.out.println();
        ...
        for (int j = 0; j < processed.size; j++) {
            if (!MyMatrix.compare(x.getArray()[j][0], processed.rhs[j][0], epsilon)) {
                ...
                System.out.println(String.format("%.1f",x.getArray()[j][0]) + " " + String.format("%.1f",processed.rhs[j][0]));
                System.exit(0);
            }
        }
    }
